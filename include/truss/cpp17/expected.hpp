/// @file expected.hpp
/// @brief This file holds Truss's `expected<T,E>` polyfill, for
///        standards that predate C++23. This polyfill is built from
///        scratch.
///
///        Unlike `std::optional`, `std::expected` does not exist at
///        all before C++23. So there is no STL type for Truss to add
///        free functions onto (compare truss/cpp17/optional.hpp).
...
///        rationale. See docs/adr/0001-namespace-and-export-scheme.md
///        for the namespace rule this file follows. See
///        docs/adr/0008-best-effort-head-standard.md for the bar
///        Deck's alias-selection (deck/cpp17/expected.hpp) must
///        clear.
///
/// `bridge::truss::expected<T,E>` is always this polyfill, regardless
/// of standard or toolchain. Truss never itself passes through to
/// `std::expected`, even under C++23, where the real type is
...
/// @brief This tag type selects `expected`'s error-constructing
///        constructor overloads. This tag type mirrors
///        `std::in_place_t`. This tag type matches `std::unexpect_t`.
/// @see https://en.cppreference.com/w/cpp/utility/expected
struct unexpect_t {
    /// @brief This default constructor is `explicit`. Without
    ///        `explicit`, `unexpect_t{}` could happen through
    ///        copy-list-initialization from `{}`, in a context that
    ///        expects some other tag type.
    explicit unexpect_t() = default;
};
/// @brief This is the canonical `unexpect_t` instance. You pass this
///        instance to select `expected`'s in-place-error
///        constructors. This instance matches `std::unexpect`.
/// @see https://en.cppreference.com/w/cpp/utility/expected
inline constexpr unexpect_t unexpect{};

/// @brief This class wraps an error value of type `E`. This class
///        matches `std::unexpected`.
///
///        You construct this class explicitly, then pass it to
///        `expected`'s converting constructors. You can also return
///        this class directly, from a function that reports failure.
...
public:
    /// @brief This copies the wrapped error.
    constexpr unexpected(const unexpected&) = default;
    /// @brief This moves the wrapped error.
    constexpr unexpected(unexpected&&) = default;

    /// @brief This constructor builds the wrapped error directly from
    ///        `e`.
    ...
    /// @brief This constructor builds the wrapped error in place,
    ///        from `args`.
    ...
    /// @brief This constructor builds the wrapped error in place,
    ///        from an initializer list plus `args`.
    ...
    /// @brief This compares the wrapped errors for equality.
    ...
    /// @brief This compares the wrapped errors for inequality.
    ///
    ///        `std::unexpected` only defines `operator==`. C++20
    ///        rewrites `!=` automatically from `operator==`. C++17,
    ///        this header's floor, does not have this automatic
    ///        rewrite. So this polyfill defines `!=` explicitly, to
    ///        match usability under passthrough. A check confirmed
    ///        that `!=` alone genuinely fails to compile without
    ///        this, under `-std=c++17`.
    /// @param lhs The left-hand `unexpected`.
    /// @param rhs The right-hand `unexpected`, possibly of a different
    ///            error type.
    ...
    /// @brief This is the ADL swap. This function forwards to the
    ///        member @ref swap.
...
/// @brief This deduction guide deduces `unexpected<E>` from a single
///        constructor argument. This matches `std::unexpected`'s
///        deduction guide.
template <class E>
unexpected(E) -> unexpected<E>;

/// @brief `expected<T,E>::value()` throws this class, when you access
///        it without a value. This class carries a copy of the error
///        that caused the access to fail. This class matches
///        `std::bad_expected_access<E>`.
///
///        This class is forward-declared here. This lets the `void`
///        specialization below, its common base, reference it. The
///        real definition is further down.
/// @tparam E The wrapped error type.
template <class E>
class bad_expected_access;

/// @brief This is the common base of `bad_expected_access<E>`. This
///        base holds the exception message. This base matches
///        `std::bad_expected_access<void>`. Only a derived
///        `bad_expected_access<E>` can construct this base.
template <>
class bad_expected_access<void> : public std::exception {
protected:
    /// @brief This constructor default-constructs the base.
    bad_expected_access() noexcept = default;
    /// @brief This copies the base.
    bad_expected_access(const bad_expected_access&) = default;
    /// @brief This moves the base.
    bad_expected_access(bad_expected_access&&) = default;
    /// @brief This copies the base, through assignment.
    /// @return `*this`.
    bad_expected_access& operator=(const bad_expected_access&) = default;
    /// @brief This moves the base, through assignment.
    /// @return `*this`.
    bad_expected_access& operator=(bad_expected_access&&) = default;
    /// @brief This destructor destroys the base.
    ~bad_expected_access() override = default;

public:
    /// @brief This is the exception message. This message is always
    ///        `"bad expected access"`.
...
/// @brief This is the real definition of @ref bad_expected_access.
///        This class publicly inherits from @ref bad_expected_access
///        "bad_expected_access<void>", through
///        `bad_expected_access_void_base`. Code can catch this class
///        without knowing `E`.
/// @see https://en.cppreference.com/w/cpp/utility/expected
template <class E>
class bad_expected_access : public bad_expected_access_void_base {
public:
    /// @brief This constructor builds from an error value, moving it
    ///        in.
    /// @param e The error value to carry.
    explicit bad_expected_access(E e) : val_(std::move(e)) {}

    /// @brief This gives lvalue access to the carried error.
    /// @return A reference to the carried error.
    E& error() & noexcept { return val_; }
    /// @brief This gives const-lvalue access to the carried error.
    /// @return A const reference to the carried error.
    const E& error() const& noexcept { return val_; }
    /// @brief This gives rvalue access to the carried error.
    /// @return An rvalue reference to the carried error.
    E&& error() && noexcept { return std::move(val_); }
    /// @brief This gives const-rvalue access to the carried error.
...
/// @brief This class is Truss's polyfilled `expected<T,E>`. This
///        class matches C++23's `std::expected<T,E>`, for standards
///        that predate it.
///
///        See the file-level docs and
///        docs/adr/0010-expected-truss-owns-the-class.md for the
///        fidelity scope this class implements. This class matches
///        the special members' deletion conditions exactly.
...
///        `explicit` here, not conditionally explicit based on
///        convertibility. Conditional `explicit(bool)` is a C++20
///        feature, unavailable on this header's C++17 floor. This
///        makes this class's surface strictly more conservative than
///        `std::expected`'s. This class only ever refuses an
///        implicit conversion that `std::expected` would allow, never
///        the reverse. So this class fails loudly at the polyfill's
///        own compile time, rather than silently diverging once
///        passthrough activates. This is the one direction
///        docs/adr/0010 actually guards against.
...
public:
    /// @brief This is the wrapped value type.
    using value_type = T;
    /// @brief This is the wrapped error type.
    using error_type = E;
    /// @brief This is the `unexpected` specialization that matches
    ///        this `expected`.
    using unexpected_type = unexpected<E>;
    /// @brief This is `expected<U,E>`: the same error type, with a
    ///        different value type. This matches
    ///        `std::expected::rebind`.
    template <class U>
    ...
    /// @brief This default constructor builds the contained value.
    ///        This constructor only participates in overload
    ///        resolution when `T` is default-constructible.
    template <class T2 = T, class = std::enable_if_t<std::is_default_constructible_v<T2>>>
    constexpr expected() : base(std::in_place) {}

    /// @brief This constructor builds a value in place, from `args`.
    ...
    /// @brief This constructor builds a value in place, from an
    ...
    /// @brief This constructor builds an error in place, from `args`.
    /// @param args Forwarded to `E`'s constructor.
    template <class... Args, class = std::enable_if_t<std::is_constructible_v<E, Args...>>>
    constexpr explicit expected(unexpect_t, Args&&... args) : base(unexpect_t{}, std::forward<Args>(args)...) {}
    /// @brief This constructor builds an error in place, from an
    ///        initializer list plus `args`.
    /// @param il Forwarded to `E`'s constructor as the first argument.
    /// @param args Forwarded to `E`'s constructor after `il`.
    template <class U, class... Args,
    ...
    /// @brief This copies `T` or `E`, matching whichever `other`
    ///        holds. This constructor is deleted unless both `T` and
    ///        `E` are copy-constructible.
    expected(const expected&) = default;
    /// @brief This moves `T` or `E`, matching whichever `other`
    ///        holds. This constructor is deleted unless both `T` and
    ///        `E` are move-constructible.
    expected(expected&&) = default;

    /// @brief This constructor builds an error from `u`'s wrapped
    ///        error.
    ...
    /// @brief This constructor builds an error, by moving `u`'s
    ///        wrapped error.
    /// @param u The `unexpected` to construct the error from.
    ...
    /// @brief This converting constructor takes an `expected<U,G>`
    ///        holding a value or error convertible to `T`/`E`.
    ///
    ///        This is a documented, simplified subset of the
    ///        standard's Constraints. This constructor matches
    ///        `is_constructible_v<T, const U&>` and
    ///        `is_constructible_v<E, const G&>`. This constructor
    ///        omits the additional defensive guards the standard
    ///        adds, against ambiguity with `T = bool` or against a
    ///        source constructible from `expected<U,G>` itself. This
    ///        omission is out of scope, the same spirit as the
    ///        exception-safety and triviality trims in
    ///        docs/adr/0010.
    ...
    /// @brief This constructor builds a value from `v`.
    ...
    /// @brief This copies whichever alternative `other` holds. This
    ///        operator is deleted unless both `T` and `E` are
    ///        copy-constructible and copy-assignable, and at least
    ///        one is nothrow-move-constructible.
    /// @return `*this`.
    expected& operator=(const expected&) = default;
    /// @brief This moves whichever alternative `other` holds. This
    ///        operator is deleted unless both `T` and `E` are
    ///        move-constructible and move-assignable, and at least
    ///        one is nothrow-move-constructible.
    /// @return `*this`.
    expected& operator=(expected&&) = default;

    /// @brief This assigns a value constructed from `v`. This
    ///        replaces whatever this held before.
    ...
    /// @brief This assigns an error constructed from `u`'s wrapped
    ///        error. This replaces whatever this held before.
    ...
    /// @brief This assigns an error, by moving `u`'s wrapped error.
    ///        This replaces whatever this held before.
    /// @param u The `unexpected` to assign the error from.
    /// @return `*this`.
    ...
    /// @brief This checks whether this holds a value, as opposed to
    ///        an error.
    ...
    /// @brief This gives lvalue access to the contained value.
    ///        `has_value()` must be true. Calling this when
    ///        `has_value()` is false is undefined behavior, matching
    ///        `std::expected`.
    ...
    /// @brief This gives lvalue access to the contained error.
    ///        `has_value()` must be false. Calling this when
    ///        `has_value()` is true is undefined behavior, matching
    ///        `std::expected`. This is not the throwing accessor; see
    ///        @ref value().
    ...
    /// @brief This returns the contained value, or `v` converted to
    ///        `T` when this holds an error instead.
    ...
    /// @brief This returns the contained error, or `g` converted to
    ///        `E` when this holds a value instead.
    ///
    ///        The original `std::expected` proposal, P0323R12,
    ///        explicitly excluded this method. This method is
    ///        present under the same `__cpp_lib_expected` value as
    ///        the rest of the type, on every ecosystem this project's
    ///        compiler matrix covers. A direct compile probe
    ///        confirmed this, before this file relied on it. See
    ...
    /// @brief If this holds a value, this method calls `f` with the
    ///        value. If this holds an error, this method does not
    ///        call `f`.
    ///
    ///        `f` must return a specialization of `expected`, with a
    ///        matching `error_type`. This method returns `f`'s result
    ///        when it calls `f`. This method returns an error copy of
    ///        that same `expected` type when it does not call `f`.
    ...
    /// @brief If this holds an error, this method calls `f` with the
    ///        error. If this holds a value, this method does not call
    ///        `f`.
    ///
    ///        `f` must return a specialization of `expected`, with a
    ///        matching `value_type`. This method returns `f`'s result
    ///        when it calls `f`. This method returns a value copy of
    ///        that same `expected` type when it does not call `f`.
    /// @param f A callable returning a specialization of `expected`.
    ...
    /// @brief If this holds a value, this method calls `f` with the
    ///        value and returns the result in a new `expected<U,E>`.
    ///        If `f` returns `void`, this method returns
    ///        `expected<void,E>` instead. If this holds an error,
    ///        this method returns an error copy.
    ///
    ///        This matches `std::expected::transform`.
    ...
    /// @brief If this holds an error, this method calls `f` with the
    ///        error and returns the result, wrapped in `unexpected`,
    ///        in a new `expected<T,G>`. If this holds a value, this
    ///        method returns a value copy of `expected<T,G>`.
    ...
    /// @brief This method destroys whatever this held, then
    ///        constructs a new value in place, from `args`. This
    ///        method requires `is_nothrow_constructible_v<T,
    ///        Args...>`, matching `std::expected` exactly.
    ///
    ///        Destroying the old alternative before constructing the
    ///        new one is only exception-safe when the new
    ///        construction cannot throw. This constraint guarantees
    ///        exactly that. A real compile error, against the real
    ///        type, confirmed this constraint is necessary: this
    ...
    /// @brief This compares two `expected`s. They are equal when both
    ///        hold a value and those values compare equal, or when
    ///        both hold an error and those errors compare equal. They
    ///        are never equal when one holds a value and the other
    ///        holds an error.
    ...
    /// @brief `std::expected` relies on C++20's automatic `!=`
    ///        rewriting from `operator==`. This rewriting is not
    ///        available under this header's C++17 floor. See the
    ///        `unexpected::operator!=` comment for why this file
    ///        defines `!=` explicitly here and throughout.
    /// @param x The left-hand `expected`.
    /// @param y The right-hand `expected`, possibly with different
    ///           value/error types.
    ...
    /// @brief This compares `x` against a raw value. They are equal
    ///        only when `x` holds a value and it compares equal to
    ///        `v`.
    ...
    /// @brief This compares `x` against a raw value for inequality.
    ///        See the `unexpected::operator!=` comment for why this
    ///        file defines this explicitly, rather than relying on
    ///        C++20 rewriting.
    /// @param x The `expected`.
    /// @param v The value to compare against.
    ...
    /// @brief This compares `x` against an `unexpected<G>`. They are
    ///        equal only when `x` holds an error and it compares
    ///        equal to `e`'s error.
    /// @param x The `expected`.
    ...
    /// @brief This compares `x` against an `unexpected<G>` for
    ///        inequality. See the `unexpected::operator!=` comment
    ///        for why this file defines this explicitly, rather than
    ///        relying on C++20 rewriting.
    /// @param x The `expected`.
    /// @param e The `unexpected` to compare against.
    ...
    /// @brief This swaps the contents of `*this` and `other`. This
    ///        includes swapping across differing alternatives: a
    ///        value swapped with an error.
...
/// @brief This is the partial specialization of `expected`, for
///        `T = void`. This specialization has no value alternative
///        to store. So construction, `operator*`, `value()`, and
///        `value_or` all drop their value-carrying parameters.
///        `value_or` is dropped entirely: there is no value to fall
///        back to. The monadic operations drop their value-carrying
///        parameters the same way.
///
///        Otherwise, this specialization mirrors the primary
///        template's API and fidelity scope, member for member:
///        constructors, assignment, `error()`, `error_or`,
///        `and_then`, `or_else`, `transform`, `transform_error`,
///        comparisons, `emplace`, and `swap`. See the primary
///        template's own doc comments for the contract each
///        corresponding member follows.
///
/// This file does not separately generate per-member documentation
/// for this specialization. Doxygen's handling of class-template
/// partial specializations conflates member lookup with the primary
/// template. Isolating the problem confirmed this: removing this
/// specialization entirely made an unrelated error disappear, on the
/// primary template's `value_or`. This showed that Doxygen was
/// resolving `T -> void` against the primary's members while it
/// processed this specialization. This is not a defect in this code.
/// This is why the documentation-coverage gate below excludes this
/// specialization, the same posture as the layered-base
...
/// @brief This is the ADL swap. This function forwards to
///        `expected`'s member `swap`.
...
/// @brief This namespace promotes `unexpect_t`, `unexpect`,
///        `unexpected`, `bad_expected_access`, and `expected` to

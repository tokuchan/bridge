...
| `BRIDGE_RIVETS_FEATURES_LIB_SOURCE_LOCATION` | define | This macro is `#if`-usable. Its value is `__cpp_lib_source_location`, or `0` when `__cpp_lib_source_location` is not defined. | [`<version>::BRIDGE_RIVETS_FEATURES_LIB_SOURCE_LOCATION`](https://en.cppreference.com/w/cpp/feature_test) |
...
| `bridge::rivets::features::lib_source_location` | variable | This value is `__cpp_lib_source_location`, or `0` when `__cpp_lib_source_location` is not defined. | [`<version>::lib_source_location`](https://en.cppreference.com/w/cpp/feature_test) |
